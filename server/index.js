...
//   console.log("old rtsp stream api ")
//   const { url } = req.query;
...
//from gpt
app.get("/api/stream-gpt", (req, res) => {

  console.log("updated API stream v2");
  const { url } = req.query;
...
//original and well tested
// RTSP Stream endpoint
app.get('/api/stream-aa', (req, res) => {
  console.log('🎥 Starting RTSP stream v0');
  const rtspUrl = req.query.url;
...
app.get('/api/stream', (req, res) => {
  ...
  console.log('🎥 Starting H.264 RTSP proxy:', rtspUrl);
  ...
  // ✅ FFmpeg args: Direct stream copy (no re-encoding)
  const ffmpegArgs = [
    '-rtsp_transport', 'tcp', // use TCP for stable RTSP
    '-i', rtspUrl,
    '-c:v', 'copy',           // ✅ no re-encode (keeps original quality)
    '-aspect', '16:9',
    '-an',                    // disable audio (optional)
    '-f', 'mp4',
    // these flags are ESSENTIAL for browser streaming
    '-movflags', 'frag_keyframe+empty_moov+default_base_moof+faststart',
    '-fflags', '+genpts+flush_packets+ignidx',
    '-avoid_negative_ts', 'make_zero',
    'pipe:1',
  ...
  // 🔁 Stream video data directly to response
  ffmpeg.stdout.pipe(res);

  // 🧩 Log FFmpeg info/errors
  ffmpeg.stderr.on('data', (data) => {
    const msg = data.toString();
    if (msg.includes('error') || msg.includes('Error')) {
      console.error('⚠️ FFmpeg error:', msg);
    }
  });
