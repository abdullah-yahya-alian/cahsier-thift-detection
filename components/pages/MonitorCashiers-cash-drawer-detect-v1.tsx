...
const MonitorCashiers: React.FC = () => {
    ...
    // const cashDrawerROIRef = useRef<NamedROI>({ x: 0.43, y: 0.25, w: 0.25, h: 0.35, name: 'Cash Drawer' }); // normalized ROI
    const cashDrawerROIRef = useRef<NamedROI | null>(null);
    const drawerROISmoothRef = useRef<NamedROI | null>(null);


    const deviceBBoxRef = useRef<{ x: number, y: number, w: number, h: number } | null>(null);
    const drawerOpenRef = useRef<boolean>(false);
    const deviceTemplateRef = useRef<ImageData | null>(null);
    const lowResCanvasRef = useRef<HTMLCanvasElement | null>(null);
    const bootstrappedRef = useRef<boolean>(false);
    const trackFrameCountRef = useRef<number>(0);
    const drawerOpenScoreRef = useRef(0); // 0..N frames
    const drawerBaselineRef = useRef<{ dark: number, grad: number, n: number } | null>(null);

    const fingertipHistoryRef = useRef<{ x: number, y: number, t: number }[]>([]);
    const lastROIFallbackSetRef = useRef<number>(0);
    const lastGoodDeviceRef = useRef<{ x: number, y: number, w: number, h: number } | null>(null);
    ...
    //******** cash drawer detect functions 

    function setDrawerROIFromDevice(devicePx: { x: number, y: number, w: number, h: number }, W: number, H: number) {
        // put ROI just below the device screen/terminal
        const roiPx = {
            x: devicePx.x,
            y: Math.floor(devicePx.y + devicePx.h * 0.92),
            w: devicePx.w,
            h: Math.floor(devicePx.h * 0.35)
        };
        // clamp to canvas
        roiPx.x = clamp(roiPx.x, 0, Math.max(0, W - 1));
        roiPx.y = clamp(roiPx.y, 0, Math.max(0, H - 1));
        roiPx.w = clamp(roiPx.w, 20, Math.max(20, W - roiPx.x));
        roiPx.h = clamp(roiPx.h, 20, Math.max(20, H - roiPx.y));

        // convert to normalized
        const target: NamedROI = {
            x: roiPx.x / W,
            y: roiPx.y / H,
            w: roiPx.w / W,
            h: roiPx.h / H,
        ...
        const alpha = 0.2;
        if (!drawerROISmoothRef.current) {
            drawerROISmoothRef.current = target;
        } else {
            const prev = drawerROISmoothRef.current;
            drawerROISmoothRef.current = {
                name: 'Cash Drawer',
        ...
        cashDrawerROIRef.current = drawerROISmoothRef.current;
    }
    ...
    function toGray(data: Uint8ClampedArray) {
        ...
        for (let i = 0, j = 0; i < n; i++, j += 4) {
            // BT.709 luminance
            g[i] = (0.2126 * data[j] + 0.7152 * data[j + 1] + 0.0722 * data[j + 2]) | 0;
        }
        return g;
    }

    function extractPatch(img: Uint8Array, iw: number, ih: number, x: number, y: number, w: number, h: number) {
        const rx = clamp(x, 0, iw - 1), ry = clamp(y, 0, ih - 1);
        const rw = clamp(w, 1, iw - rx), rh = clamp(h, 1, ih - ry);
        const out = new Uint8Array(rw * rh);
        for (let j = 0; j < rh; j++) {
            out.set(img.subarray((ry + j) * iw + rx, (ry + j) * iw + rx + rw), j * rw);
        }
        return { data: out, w: rw, h: rh };
    }

    // normalized cross-correlation on two same-sized grayscale patches
    function ncc(a: Uint8Array, b: Uint8Array) {
        let n = a.length, sa = 0, sb = 0, sqa = 0, sqb = 0, sab = 0;
        for (let i = 0; i < n; i++) {
            const va = a[i], vb = b[i];
            sa += va; sb += vb;
            sqa += va * va; sqb += vb * vb;
        ...
        const num = sab - n * ma * mb;
        const den = Math.sqrt((sqa - n * ma * ma) * (sqb - n * mb * mb)) + 1e-6;
        return num / den;
    }

    function downsampleInto(video: HTMLVideoElement, out: HTMLCanvasElement, scale = 0.33) {
        ...
        return { gray: toGray(imgData.data), w, h };
    }

    // search device template around previous location in low-res image
    function searchAround(lowRes: { gray: Uint8Array, w: number, h: number }, tpl: Uint8Array, tw: number, th: number, cx: number, cy: number, searchRad: number) {
        let best = { x: cx, y: cy, score: -1 };
        // integer step search
        for (let dy = -searchRad; dy <= searchRad; dy += 2) {
            for (let dx = -searchRad; dx <= searchRad; dx += 2) {
                const x = clamp(cx + dx - (tw >> 1), 0, lowRes.w - tw);
                const y = clamp(cy + dy - (th >> 1), 0, lowRes.h - th);
                const cand = extractPatch(lowRes.gray, lowRes.w, lowRes.h, x, y, tw, th);
                const s = ncc(tpl, cand.data);
    ...
    // rough drawer-open heuristic using darkness + edges in a band below device
    function drawerOpenHeuristic(lowRes: { gray: Uint8Array, w: number, h: number }, dev: { x: number, y: number, w: number, h: number }, bandRelTop = 0.90, bandRelBottom = 1.25) {
        // the band just below device bottom, relative to device box
        const y0 = Math.floor((dev.y + dev.h * bandRelTop));
        const y1 = Math.min(lowRes.h - 1, Math.floor(dev.y + dev.h * bandRelBottom));
        const x0 = Math.max(0, dev.x), x1 = Math.min(lowRes.w - 1, dev.x + dev.w);

        const width = x1 - x0;
        const height = Math.max(1, y1 - y0);
        if (width < 8 || height < 4) return false;

        // metrics: dark ratio and simple horizontal gradient magnitude
        let dark = 0, total = 0, gradSum = 0;
        for (let y = y0 + 1; y < y1 - 1; y++) {
            for (let x = x0 + 1; x < x1 - 1; x++) {
                const idx = y * lowRes.w + x;
                const v = lowRes.gray[idx];
                if (v < 60) dark++;
                total++;
                const gx = (lowRes.gray[idx + 1] - lowRes.gray[idx - 1]);
                const gy = (lowRes.gray[idx + lowRes.w] - lowRes.gray[idx - lowRes.w]);
                gradSum += Math.abs(gx) + Math.abs(gy);
            }
        }
        const darkRatio = dark / (total + 1e-6);
        const gradAvg = gradSum / (total + 1e-6);

        // tune thresholds: dark area appears when drawer is open + edges increase
        const open = darkRatio > 0.22 && gradAvg > 8;
        return open;
    }

    function inflateRect(r: { x: number, y: number, w: number, h: number }, s: number, W: number, H: number) {
        const nx = clamp(r.x - r.w * s, 0, W - 1);
        const ny = clamp(r.y - r.h * s, 0, H - 1);
        const nw = clamp(r.w * (1 + 2 * s), 4, W - nx);
        const nh = clamp(r.h * (1 + 2 * s), 4, H - ny);
        return { x: nx, y: ny, w: nw, h: nh };
    }

    function scaleRect(r: { x: number, y: number, w: number, h: number }, s: number) {
        return { x: Math.floor(r.x * s), y: Math.floor(r.y * s), w: Math.floor(r.w * s), h: Math.floor(r.h * s) };
    }

    function unscaleRect(r: { x: number, y: number, w: number, h: number }, s: number) {
        return { x: Math.floor(r.x / s), y: Math.floor(r.y / s), w: Math.floor(r.w / s), h: Math.floor(r.h / s) };
    }

    //It runs only until it finds the device.
    function tryBootstrapDeviceOld(video: HTMLVideoElement, W: number, H: number) {
        if (!objectDetectorRef.current) return false;
        const det = objectDetectorRef.current.detectForVideo(video, performance.now());
        const D = det?.detections ?? [];
        if (!D.length) return false;

        // pick only likely register-related classes
        const good = [];
        for (const d of D) {
            const cat = d.categories?.[0];
            if (!cat) continue;
            const name = (cat.categoryName || '').toLowerCase();
            if (!['keyboard', 'laptop', 'tv', 'monitor', 'computer'].some(k => name.includes(k))) continue;
            if (!d.boundingBox) continue;
            good.push(d.boundingBox);
        }
        if (!good.length) return false;

        // union their boxes
        let minX = +1e9, minY = +1e9, maxX = -1e9, maxY = -1e9;
        for (const bb of good) {
            minX = Math.min(minX, bb.originX);
            minY = Math.min(minY, bb.originY);
            maxX = Math.max(maxX, bb.originX + bb.width);
            maxY = Math.max(maxY, bb.originY + bb.height);
        }
        const box = { x: Math.floor(minX), y: Math.floor(minY), w: Math.floor(maxX - minX), h: Math.floor(maxY - minY) };
        // sanity
        if (box.w < 40 || box.h < 40) return false;

        deviceBBoxRef.current = box;
        bootstrappedRef.current = true;

        // optional: free the detector to save compute entirely
    ...
    function classPriority(name: string) {
        // prefer visual terminals/screens over keyboards
        // "tv" and "monitor" often capture POS/monitor; "laptop/computer" okay; "keyboard" last resort
        if (name.includes('tv') || name.includes('monitor')) return 4;
        if (name.includes('laptop') || name.includes('computer')) return 3;
        if (name.includes('keyboard')) return 1;
        return 0;
    }

    function tryBootstrapDevice(video: HTMLVideoElement, W: number, H: number) {
        if (!objectDetectorRef.current) return false;
        const det = objectDetectorRef.current.detectForVideo(video, performance.now());
        const D = det?.detections ?? [];
        if (!D.length) return false;

        let best = null as null | { bb: any, score: number, area: number, priority: number };
        for (const d of D) {
            const cat = d.categories?.[0];
            if (!cat || !d.boundingBox) continue;
            const name = (cat.categoryName || '').toLowerCase();
            const pri = classPriority(name);
            if (pri <= 0) continue;

            const bb = d.boundingBox;
            const area = Math.max(1, bb.width * bb.height);
            const score = (cat.score ?? 0) * pri * Math.sqrt(area);
            if (!best || score > best.score) {
                best = { bb, score, area, priority: pri };
            }
        }

        if (!best) {
            // optionally: fallback to keyboard if it's the only thing
            const kb = D
                .filter(d => (d.categories?.[0]?.categoryName || '').toLowerCase().includes('keyboard') && d.boundingBox)
                .sort((a, b) => (b.categories?.[0]?.score ?? 0) - (a.categories?.[0]?.score ?? 0));
            if (!kb.length) return false;
            const bb = kb[0].boundingBox!;
            if (bb.width < 40 || bb.height < 40) return false;
            deviceBBoxRef.current = { x: Math.floor(bb.originX), y: Math.floor(bb.originY), w: Math.floor(bb.width), h: Math.floor(bb.height) };
            bootstrappedRef.current = true;
    ...
    //this helper Per-frame lightweight tracking + drawer-open detection. It uses the low-res canvas, runs every 2–3 frames, very cheap.


    function updateDeviceAndDrawer(video: HTMLVideoElement, ctx: CanvasRenderingContext2D, W: number, H: number) {
        // try to bootstrap once if not set
        if (!deviceBBoxRef.current && objectDetectorRef.current && !bootstrappedRef.current) {
            tryBootstrapDevice(video, W, H);
        }

        if (!deviceBBoxRef.current) return; // nothing to track

        // downsample current frame
        const scale = 0.33;
        const low = downsampleInto(video, lowResCanvasRef.current!, scale);
        const devLR = scaleRect(deviceBBoxRef.current, scale);

        // build template if missing
        if (!deviceTemplateRef.current) {
            const patchW = Math.max(24, Math.floor(devLR.w * 0.5));
            const patchH = Math.max(24, Math.floor(devLR.h * 0.5));
            const cx = Math.floor(devLR.x + devLR.w * 0.5);
            const cy = Math.floor(devLR.y + devLR.h * 0.45);
            const sx = clamp(cx - (patchW >> 1), 0, low.w - patchW);
            const sy = clamp(cy - (patchH >> 1), 0, low.h - patchH);
            const patch = extractPatch(low.gray, low.w, low.h, sx, sy, patchW, patchH);
            deviceTemplateRef.current = new ImageData(patchW, patchH);
            // store as grayscale in ImageData.data.r to reuse array
            // but for NCC we’ll keep raw Uint8Array copy:
            (deviceTemplateRef.current as any)._gray = patch.data;
            (deviceTemplateRef.current as any)._w = patchW;
            (deviceTemplateRef.current as any)._h = patchH;
        }

        // track every 2nd frame to save CPU
        trackFrameCountRef.current = (trackFrameCountRef.current + 1) % 2;
        if (trackFrameCountRef.current === 0) {
            const tpl = (deviceTemplateRef.current as any)._gray as Uint8Array;
            const tw = (deviceTemplateRef.current as any)._w as number;
            const th = (deviceTemplateRef.current as any)._h as number;

            const cx = Math.floor(devLR.x + devLR.w * 0.5);
            const cy = Math.floor(devLR.y + devLR.h * 0.45);
            const searchRad = Math.max(6, Math.floor(Math.max(devLR.w, devLR.h) * 0.15));
            const best = searchAround(low, tpl, tw, th, cx, cy, searchRad);

            // update device box center from match
            const nDevLR = { ...devLR };
            nDevLR.x = best.x - (tw >> 1) + (devLR.w >> 1);
            nDevLR.y = best.y - (th >> 1) + Math.floor(devLR.h * 0.05);
            // clamp
            nDevLR.x = clamp(nDevLR.x, 0, low.w - nDevLR.w);
            nDevLR.y = clamp(nDevLR.y, 0, low.h - nDevLR.h);

            deviceBBoxRef.current = unscaleRect(nDevLR, scale);
            if (deviceBBoxRef.current) lastGoodDeviceRef.current = { ...deviceBBoxRef.current };

            // setDrawerROIFromDevice(deviceBBoxRef.current, W, H);


            if (deviceBBoxRef.current) {
                setDrawerROIFromDevice(deviceBBoxRef.current, W, H);
            }
        }

        // drawer open heuristic in low-res space
        const devLR2 = scaleRect(deviceBBoxRef.current, scale);
        const inflated = inflateRect(devLR2, 0.05, low.w, low.h);
        // const isOpen = drawerOpenHeuristic(low, inflated, 0.92, 1.28);
        const isOpen = drawerOpenHeuristicAdaptive(low, inflated, 0.92, 1.28);
        // drawerOpenRef.current = isOpen;
        let _is_really_open = isOpen;
        if (isOpen) {
            drawerOpenScoreRef.current = Math.min(12, drawerOpenScoreRef.current + 2);
        } else {
            drawerOpenScoreRef.current = Math.max(0, drawerOpenScoreRef.current - 1);
        }
        _is_really_open = drawerOpenScoreRef.current >= 5;
        drawerOpenRef.current = drawerOpenScoreRef.current >= 5; // require a few frames to say "open"



        // draw overlays (device always, drawer only if open)
        ctx.save();
        // device
        ctx.strokeStyle = '#38bdf8';
        ctx.lineWidth = 2;
        const d = deviceBBoxRef.current;
        ctx.strokeRect(d.x, d.y, d.w, d.h);
        drawLabel(ctx, d.x, d.y, 'Cash Device', 'rgba(7, 89, 133, 0.7)', '#e0f2fe');

        // if (isOpen) {
        if (_is_really_open) {
            // drawer area (band below device bottom)
            const y0 = Math.floor(d.y + d.h * 0.92);
            const y1 = Math.floor(d.y + d.h * 1.28);
            ctx.strokeStyle = '#f59e0b';
            ctx.setLineDash([6, 4]);
            ctx.strokeRect(d.x, y0, d.w, clamp(y1 - y0, 4, H - y0));
            drawLabel(ctx, d.x, y0, 'Drawer: OPEN', 'rgba(120, 53, 15, 0.75)', '#ffd38b');
            ctx.setLineDash([]);
        }
        ctx.restore();

        // expose open state if you want to use it elsewhere (optional)
        (window as any).__drawerOpen = drawerOpenRef.current;
    }


    function drawerOpenHeuristicAdaptive(lowRes, dev, bandRelTop = 0.92, bandRelBottom = 1.28) {
        const y0 = Math.floor((dev.y + dev.h * bandRelTop));
        const y1 = Math.min(lowRes.h - 1, Math.floor(dev.y + dev.h * bandRelBottom));
        const x0 = Math.max(0, dev.x), x1 = Math.min(lowRes.w - 1, dev.x + dev.w);
        const width = x1 - x0, height = Math.max(1, y1 - y0);
        if (width < 8 || height < 4) return false;
        ...
        // learn baseline while drawer thought to be closed
        if (!drawerOpenRef.current) {
            if (!drawerBaselineRef.current) drawerBaselineRef.current = { dark: 0, grad: 0, n: 0 };
            const b = drawerBaselineRef.current;
            // EMA to be robust
            b.dark = b.n === 0 ? darkRatio : b.dark * 0.95 + darkRatio * 0.05;
            b.grad = b.n === 0 ? gradAvg : b.grad * 0.95 + gradAvg * 0.05;
            b.n++;
        }

        const base = drawerBaselineRef.current;
        // if baseline exists, use relative increase; else fall back to absolute thresholds
        if (base && base.n > 20) {
            const darkLift = darkRatio - base.dark;   // how much darker than baseline
            const gradLift = gradAvg - base.grad;     // how many more edges/texture
            return darkLift > 0.10 && gradLift > 3.0;
        } else {
            return darkRatio > 0.22 && gradAvg > 8; // initial static thresholds
        }
    }


    //********end of cash drawer detect functions */
            ...
            try {
                ...
                objectDetectorRef.current = await (window as any).mp.tasks.vision.ObjectDetector.createFromOptions(vision, {
                    baseOptions: {
                        ...
                        // modelAssetPath: 'https://storage.googleapis.com/mediapipe-models/object_detector/efficientdet_lite2/float16/1/efficientdet_lite2.task',
                        // modelAssetPath: 'https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_full/float16/1/pose_landmarker_full.task',
    ...
    const predictLoop = useCallback(() => {
        ...
        /**
         * 
         * - Update any code that uses drawer ROI to guard against null:
    const drawer = cashDrawerROIRef.current;
    if (drawer && anyPointInROI(fingerTips, drawer)) { ... }
            ...
            if (handRes?.landmarks?.length) {
                for (const handLm of handRes.landmarks) {
                    ...
                    for (const tip of fingerTips) {
                        // keep only points likely around counter region (lower 70% of frame) to avoid false clusters up top
                        if (tip.y > 0.30) {
                            fingertipHistoryRef.current.push({ x: tip.x, y: tip.y, t: now });
                        }
                    }
                    // keep last 4 seconds
                    const cutoff = now - 4000;
                    fingertipHistoryRef.current = fingertipHistoryRef.current.filter(p => p.t >= cutoff);

                    // if we have no device bbox and no drawer ROI yet, try to infer one from fingertips density
                    if (!deviceBBoxRef.current && !cashDrawerROIRef.current && fingertipHistoryRef.current.length > 50 && now - lastROIFallbackSetRef.current > 1500) {
                        // compute bounding box of the densest cluster via simple grid
                        const pts = fingertipHistoryRef.current;
                        // coarse grid accumulation
                        const grid = new Map<string, number>();
                        const cell = 0.05; // 5% of image
                        for (const p of pts) {
                        ...
                        for (const [k, v] of grid.entries()) if (v > bestCount) { bestCount = v; bestKey = k; }
                        if (bestKey && bestCount >= 12) {
                            const [gx, gy] = bestKey.split(',').map(Number);
                            const cx = (gx + 0.5) * cell;
                            const cy = (gy + 0.5) * cell;
                            // define a ROI around that cell with some margin
                            const roi: NamedROI = {
                                name: 'Cash Drawer',
                                x: clamp(cx - 0.12, 0, 0.95),
                                y: clamp(cy - 0.08, 0, 0.95),
                                w: 0.24,
                                h: 0.16
                            };
                            // smooth into the main ROI
                            if (!drawerROISmoothRef.current) drawerROISmoothRef.current = roi;
                            else {
                                const prev = drawerROISmoothRef.current;
                                drawerROISmoothRef.current = {
                                    name: 'Cash Drawer',
                                    x: prev.x + (roi.x - prev.x) * 0.3,
                                    y: prev.y + (roi.y - prev.y) * 0.3,
                                    w: prev.w + (roi.w - prev.w) * 0.3,
                                    h: prev.h + (roi.h - prev.h) * 0.3,
                                };
                            }
                            cashDrawerROIRef.current = drawerROISmoothRef.current;
                            lastROIFallbackSetRef.current = now;
                        }
                    }
                }
            }
        ...
        if (videoRef.current) {
            updateDeviceAndDrawer(videoRef.current, canvasCtx, canvas.width, canvas.height);
        }
